...
class HomeNetworkKeyPairGen(object):
    ...
    def validate_public_key(self):
        """
        Validate the ProfileB public key
        """
        public_key = self.home_network_key_pair.home_network_public_key
        if len(public_key) != 33:
            raise ValueError("Invalid compressed public key length:", len(public_key))
        if public_key[0] != 0x02:
            raise ValueError("Invalid compression indicator:", public_key[0])

        # Validate the remaining bytes if needed
        print("Public key length is " + str(len(public_key))+" and value is "+str(public_key))
        return public_key
...
def generate_profileb_keypair_and_print():
    ...
    home_network_keypair_gen.validate_public_key()
